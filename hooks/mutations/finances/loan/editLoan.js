...
export default function UseEditLoan() {
    ...
    const editLoan = async ({ id, formData }) => {
        ...
        return await axios.post(
            `https://hr-backend-talo.com/api/loan/${id}/edit-installments`,
            formData,
            {
                headers: {
    ...
    return useMutation(editLoan, {
        onSuccess: async (res, variables, context) => {
            successToaster("Loan updated successfully!");
            queryClient.refetchQueries(["AllManagerLoanList"]);
            queryClient.refetchQueries(["IndividualLoanDetails"]);
        },
        onError: (err, variables, context) => {
            let errorMessage = "Error in updating loan. ";
