...
export default function UseRequestAdvanceSalary() {
  const queryClient = useQueryClient()

  const advanceSalary = async (formData) => {
    ...
    return await axios.post(
      `https://hr-backend-talo.com/api/early-salary/createESRequest`,
      formData,
      {
        headers: {
  ...
  return useMutation(advanceSalary, {
    ...
    onError: (err, variables, context) => {
      let errorMessage = "Failed to request advance salary.";
