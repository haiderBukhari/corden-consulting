...
export default function UseApprovePayrollConfigrationApproval() {
  const queryClient = useQueryClient()

  const updatePayrollRequest = async ({id,data}) => {
    ...
    return await axios.post(
      `https://hr-backend-talo.com/api/payroll-configurations/update-status/${id}`,
      data,
      {
        headers: {
  ...
  return useMutation(updatePayrollRequest, {
    onSuccess: async (res, variables, context) => {
      queryClient.refetchQueries(['PayrollConfigrationApprovalList']);
      queryClient.refetchQueries(['PayrollConfigrationData']);
      
      successToaster("Payroll Configration Approved/rejected Successfully");
    },
