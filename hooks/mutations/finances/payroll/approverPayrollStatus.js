...
export default function UseUpdatePayrollRequest() {
  const queryClient = useQueryClient();

  const updatePayrollRequest = async ({ id,formData }) => {
    ...
    return await axios.post(
      `https://hr-backend-talo.com/api/payroll-requests/status-update/${id}`,
      formData,
      {
        headers: {
  ...
  return useMutation(updatePayrollRequest, {
    onSuccess: async (res, variables, context) => {
      ...
      successToaster("Payroll request updated successfully.");
    },
    onError: (err, variables, context) => {
      let errorMessage = "Error in updating request!";
