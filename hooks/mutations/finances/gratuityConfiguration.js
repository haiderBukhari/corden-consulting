...
export default function UseUpdateGratuityConfiguiration() {
    ...
    const editRegionVariable = async ({ id, formData }) => {
        ...
        return await axios.post(
            `https://hr-backend-talo.com/api/gratuity-configurations/${id}`,
            formData,
            {
                headers: {
    ...
    return useMutation(editRegionVariable, {
        onSuccess: async (res, variables, context) => {
            successToaster("Updated Grutity Configuiration successfully!");
            queryClient.refetchQueries(["GratuityDetail"]);
            queryClient.refetchQueries(["GratuityConfiguration"]);
            queryClient.refetchQueries(["profile"]);
        },
        onError: (err, variables, context) => {
            let errorMessage = "Error in updating Grutity . ";
