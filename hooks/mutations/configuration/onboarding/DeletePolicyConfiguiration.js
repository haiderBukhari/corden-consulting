...
export default function UseDeletePolicyConfiguration() {
  ...
  const deletePolicyConfiguration = async (id) => {
    ...
    return await axios.delete(
      `https://hr-backend-talo.com/api/policies/${id}`,
      {
        headers: {
    ...
    return useMutation(deletePolicyConfiguration, {
    onSuccess: async (res, variables, context) => {

      queryClient.refetchQueries(["policies"]);
      // queryClient.refetchQueries(["workingHours"]);
      successToaster("Policy Deleted Successfully")

    },

    onError: (err, variables, context) => {
      let errorMessage = "Error in deleting policy configuration! ";
