...
export default function UseCreatePolicyConfiguration() {
  const queryClient = useQueryClient()

  const createPolicyConfiguration = async (data) => {
    ...
    return await axios.post(
      `https://hr-backend-talo.com/api/policies`,
      data,
      {
        headers: {
  ...
  return useMutation(createPolicyConfiguration, {
    onSuccess: async (res, variables, context) => {

      queryClient.refetchQueries(["policies"]);
      // queryClient.refetchQueries(["workingHours"]);
      successToaster("Policy Created Successfully")

    },

    onError: (err, variables, context) => {
      let errorMessage = "Error in creating policy configuration! ";
