...
export default function UseInitiatePayroll() {

  const queryClient = useQueryClient()

  const initiatePayroll = async (formData) => {
    ...
    return await axios.post(
      `https://hr-backend-talo.com/api/InitiatePayroll`,
      {},
      {
        headers: {
  ...
  return useMutation(initiatePayroll, {
    onSuccess: async (res, variables, context) => {
      ...
      successToaster("Payroll initiated successfully!")
    },
    onError: (err, variables, context) => {
      let errorMessage = "Error in initiating payroll.";
