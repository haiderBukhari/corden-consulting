...
import UseEditPosition from '../../../../hooks/mutations/admin/editPosition';
import { UseGetUsers } from '../../../../hooks/query/admin/getUserList';
import { successToaster } from '../../../../utils/toaster';
import DataLoader from '../../../ui/dataLoader';
import Image from 'next/image';

const EditPositionComponent = ({ positionData, setEditPosition }) => {
    const { mutate, isLoading } = UseEditPosition();
    const { data: UserList, isLoading: isLoadingUsers } = UseGetUsers();
    const [searchQuery, setSearchQuery] = useState('');


    useEffect(() => {
        if (UserList && positionData.team_members) {
            const preSelectedIds = new Set(positionData.team_members.map(member => member.id));
            const preSelectedMembers = UserList.filter(user => preSelectedIds.has(user.id));
            const remainingMembers = UserList.filter(user => !preSelectedIds.has(user.id) && user.position === null);
            setFilteredMembers([...preSelectedMembers, ...remainingMembers]);
        }
    }, [UserList, positionData]);
    ...
    const formik = useFormik({
        initialValues: {
            ...
            selectedMembers: positionData.team_members ? positionData.team_members.map(member => member.id) : [],
        },
        validationSchema,
        onSubmit: (values) => {
            const formData = new FormData()
            formData.append("position_id", positionData.id)
            formData.append("name", values.positionName)
            formData.append("members", JSON.stringify(values.selectedMembers));
            mutate(formData, {
                onSuccess: () => {
                    successToaster("Position updated successfully!")
    ...
    return (
        <div className=" min-h-screen">
            <button onClick={() => { setEditPosition(false) }} type='button' className='flex space-x-2 items-center px-2 py-1 text-white bg-primary rounded-xl  mb-3'>
                <ArrowLeft className='text-white h-5 w-5' />
                <span>Back</span>
            </button>
            <form onSubmit={formik.handleSubmit}>
                <div className="mb-4">
                    <label htmlFor="positionName" className="block mb-2">Position Name</label>
                    <input
                        type="text"
                        id="positionName"
                        name="positionName"
                    ...
                    {formik.errors.positionName && formik.touched.positionName && (
                        <div className="text-darkred">{formik.errors.positionName}</div>
                    )}
                </div>
                ...
                <div className="flex justify-end my-3">
                    <button type="button" onClick={() => { setEditPosition(null) }} className="px-4 py-2 bg-secondary text-white rounded-md mr-4">
                        Cancel
                    </button>
