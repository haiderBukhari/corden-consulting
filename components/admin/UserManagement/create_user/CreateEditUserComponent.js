...
import UseEditStaffUser from '../../../../hooks/mutations/admin/editUser';
...
const CreateEditUserComponent = ({ role }) => {
  ...
  const editUser = UseEditStaffUser();
  ...
  const { data: staffUserData, isLoading: isLoadingStaffUser } = UseGetStaffUserById(user_id);
  ...
  useEffect(() => {
    if (isEditUser && staffUserData && rolesList) {

      const userRole = rolesList.find(
        (r) => r.name.toLowerCase() === staffUserData.role.toLowerCase()
      );

      // Convert "14 Oct 2024" to "2024-10-14"

      const newInitialValues = {
        firstName: staffUserData.fname || '',
        middleName: staffUserData.middle_name || '',  // Middle name
        lastName: staffUserData.lname || '',
        email: staffUserData.email || '',
        gender: staffUserData.gender || '',
        phoneNumber: staffUserData.phone_number || '',
        nextOfKinName: staffUserData.next_of_kin || '',
        contact: staffUserData.kin_contact || '',
        nextOfKinRelation: staffUserData.kin_relation || '',
        position: staffUserData.position?.id || '',
        branch: staffUserData.branch?.id || '',
        location: staffUserData.location?.id || '',
        salary: staffUserData.current_salary || '',
        role: userRole ? userRole.role_id : '',
        department: staffUserData.department?.id || '',
        team: staffUserData.team_obj?.id || '',
        reportsTo: staffUserData?.reports_to || '',
        joiningDate: staffUserData?.date_of_joining,
        restrictedApi: staffUserData?.is_restricted_IP === 1,
        workers: staffUserData?.is_worker === 1,
        financeAccessManager: staffUserData?.access_finance_section === 1,
        bankName: staffUserData?.bank_name || '',
        accountNumber: staffUserData?.account_number || '',
        shiftId: String(staffUserData?.shift_id),
        password: '',
        confirmPassword: ''
      };
  ...
  }, [isEditUser, staffUserData, rolesList]);
  ...
  const validationSchema = Yup.object().shape({
    ...
    salary: role === 'manager' ? Yup.number().required('Salary is required') : Yup.number(),
    role: Yup.string().required('Role is required'),
    department: role !== 'department_head' ? Yup.string().required('Department is required') : Yup.string(),
    team: Yup.string(),
    ...
    password: !isEditUser ? Yup.string().min(8, 'Password must be at least 8 characters').required('Password is required') : Yup.string(),
    confirmPassword: !isEditUser ? Yup.string()
      ...
      if (isEditUser) {
        ...
        editUser.mutate(payload, {
          onSuccess: () => {
            ...
            resetForm();
            router.push(role == 'admin' ? `/${role}/staff_directory/list` : `/workforce/people/${user_id}/detail-page`);
          }
        });
      } else {
        createUser.mutate(payload, {
          onSuccess: () => {
            ...
            router.push(role == 'admin' ? `/${role}/staff_directory/list` : `/workforce/people/list`);
            resetForm();
          }
        });
      }
  ...
  return (
    <div>
      {positionsData && branchData && departmentData && rolesList && locationsData && !isLoadingDepartments && !isLoadingPositions && !isLoadingRoles && !isLoadingBranches && !isLoadingLocations && !isLoadingTeams && (!isEditUser || (isEditUser && !isLoadingStaffUser)) ? (
        <div className='text-default_text min-h-screen'>
          <div className='rounded-lg mx-5 max-w-6xl'>
            <button
              onClick={() => {
                const isStaffPage = router.asPath.includes("staff_directory"); // Check if "staff" is in the URL
                router.push(
                  isEditUser
                    ? role === "admin"
                      ? `/${role}/staff_directory/list` // Admin always goes to staff directory
                      : isStaffPage
                      ? `/workforce/staff_directory/list` // If "staff" in URL, go to Staff Directory
                      
                      : `/workforce/people/${user_id}/detail-page`
                    : `/${role}/dashboard`
                );
              }}
              type="button"
            ...
            <form onSubmit={formik.handleSubmit}>
              ...
              <div className='grid grid-cols-3 gap-4 mt-4'>
                ...
                <div className='mb-4'>
                  <label htmlFor='joiningDate' className='block'>Date of Joining</label>
                  <input
                    type='date'
                    id='joiningDate'
                    name='joiningDate'
              ...
              <div className='grid grid-cols-3 gap-4 mt-4'>
                ...
                <div className=' mb-4 '>
                  ...
                  >
                    ...
                    {UserList
                      ?.filter((user) => user.role === "manager" && user.id !== staffUserData?.id)
                ...
                {role === 'manager' && (
                  <div className='mb-4 relative'>
                    ...
                    <input
                      ...
                      onChange={formik.handleChange}
                      className='rounded-lg text-sm w-full bg-white border text-default_text border-gray-300 p-2 mt-1 pr-10'
                    />
                    <div
                    ...
                    >
                      {showSalary ? (
                        <FaEye className="text-red-500 h-5 w-5 mt-1" />
                      ) : (
                        <FaEyeSlash className="text-red-500 h-5 w-5 mt-1" />
                      )}
                    </div>
              ...
              <div className="flex justify-end space-x-3 pt-12 pb-4">
                <button
                  ...
                  onClick={() => { router.push(isEditUser ? role == 'admin' ? `/${role}/staff_directory/list` : role == 'HR' ? `/workforce/staff_directory/list` : `/workforce/people/${user_id}/detail-page` : `/${role}/dashboard`) }}
                >
                ...
                </button>
                {
                  isEditUser ?
                    <button
                      type="submit"
                      className="flex items-center justify-center rounded-xl text-center px-9 py-2 bg-primary text-white border text-sm disabled:bg-opacity-80"
                    >
                      <CiFloppyDisk className='text-base mr-1' />
                      {editUser.isLoading ? <ButtonLoader text={'Saving...'} /> : "Save"}
                    </button>
                    :
                    <button
                      type="submit"
                    ...
                    >
                      <CiFloppyDisk className='text-base mr-1' />
                      {createUser.isLoading ? <ButtonLoader text={'Creating...'} /> : "Create"}
                    </button>
                }
              </div>
            </form>
