...
export default function StaffListComponent({ isDashBoard,role }) {
  const { data: allStaffUsers, isLoading,  } = UseGetAllUsers();
  const [searchTerm, setSearchTerm] = useState('');
  const [sortCriteria, setSortCriteria] = useState('name');
  const [deactivateModalVisible, setDeactiveModalVisible] = useState(false);
  ...
  const closeDeactivateeModal = () => {
    setUserToDeactivate(null);
    setDeactiveModalVisible(false);
  };
  ...
  return (
    <div>
                    ...
                    <tbody>
                      {filteredUsers.map((user, index) => (
                        <tr key={index} className={`${index === filteredUsers.length - 1 ? '' : 'border-b border-gray-100'}`}>
                          ...
                          <td className="px-2 py-3 text-left text_default_text text-sm capitalize">{user?.department?.departments_name || 'No Department'}</td>
                          ...
                          <td className="px-2 py-2 text-left text_default_text text-sm flex justify-center items-center align-middle">
                            <button
                              onClick={() => { router.push( role === 'HR' ? `/workforce/staff_directory/edit_user?user_id=${user.id}` :`/admin/staff_directory/edit_user?user_id=${user.id}`) }}
                            ...
                            <button
                              ...
                              className="group relative rounded-md py-2 px-4 border border-secondary text_default_text text-base"
                            >
                              <PowerOff className='h-4 w-5' />
      ...
      <DeleteItemModal modalVisible={deactivateModalVisible} item={'user'} handleDeleteItem={handleDeactivateUser} closeModal={closeDeactivateeModal} action={'deactivate'} />
    </div>
  );
}
